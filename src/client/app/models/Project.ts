...
export class Project {
  ...
  parseContentField() {
    ...
    if (jsonContent.routes)
      this.routes = jsonContent.routes.map((val:any) => {
        return new Route().parseFromJson(val);
      });
    if (jsonContent.controllers)
      this.controllers = jsonContent.controllers.map((val:any) => {
        return new Controller().parseFromJson(val);
      });
    if (jsonContent.views)
      this.views = jsonContent.views.map((val:any) => {
        return new View().parseFromJson(val);
      });
    if (jsonContent.models)
      this.models = jsonContent.models.map((val:any) => {
        return new Model().parseFromJson(val);
      });
    if (jsonContent.gems)
      this.gems = jsonContent.gems.map((val:any) => {
        return new Gem().parseFromJson(val);
      });
  }
